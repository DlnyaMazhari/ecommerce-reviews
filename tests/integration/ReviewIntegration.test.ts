...
describe("ReviewController Integration Tests", () => {
  ...
  describe("POST /api/products/:productId/reviews", () => {
    ...
    it("should return an error if the user has already reviewed the product", async () => {
      const mockReviewData = {
      ...
      await request(app).post("/api/products/1/reviews").send(mockReviewData);
